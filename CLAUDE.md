...
A demo MCP server showcasing BranderUX — branded, interactive UI rendering inside AI chatbots. Published as `@brander/mcp-demo` on npm. Uses `@brander/mcp-tools` to register the `generate_screen` tool and adds 4 demo-specific knowledge tools on top.
